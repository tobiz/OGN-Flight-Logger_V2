...
L_SMALL = float(0.001) # of a degree
A_SMALL = float(0.01)  # of a metre, ie 1cm
V_SMALL = float(0.0)
frst_time = False
AIRFIELD = "SuttonBnk"
...
#                      			reg_no TEXT, latitude TEXT, longitude TEXT, altitude TEXT, course TEXT, speed TEXT)''')
    ...
    cursor.execute('''CREATE TABLE IF NOT EXISTS
    					flight_log2(id INTEGER PRIMARY KEY, sdate TEXT, stime TEXT, edate TEXT, etime TEXT, duration TEXT,
...
test = False
if test == True:
	datafile = open ('SB_data-20150224', 'rw')
else:
	datafile = open ('SB_data-20150224', 'w')
i = 0
try:
	while 1:
...
#			if res == -1:
		...
		if fleet_check(str(src_callsign)) == False:
...
#			print "Line ", i, " ", packet[0].orig_packet
			...
			if nprev_vals[src_callsign]['speed'] <> 0 and nvalues[src_callsign]['speed'] == 0:
				...
				cursor.execute('''SELECT sdate, stime, max_altitude FROM flight_log2 WHERE ROWID IN (SELECT max(id) FROM flight_log2 WHERE src_callsign =? )''',(src_callsign,))
				row = cursor.fetchone()
				for r in row:
...
#				end_time = datetime.strptime(fl_time,'%H:%M:%S') 
				...
				print "Flight End date and time are: ", fl_end_date, " , ", fl_end_time
				...
				fl_duration_time = fl_end_time - fl_start_time						# fl_duration_time is a string format %H:%M:%S
				fl_duration_time_str = str(fl_duration_time)
				print "Start time: ", fl_start_time, "End time: ", fl_end_time, "Duration: ", fl_duration_time, " Max altitude: ", max_altitude
				
				cursor.execute('''INSERT INTO flight_log_final(sdate, stime, edate, etime, duration, src_callsign, max_altitude, speed)
							VALUES(:sdate,:stime,:edate,:etime,:duration,:src_callsign,:max_altitude,:speed)''',
							{'sdate':start_date,'stime':start_time,'edate': fl_end_date,'etime':fl_end_time,
							'duration': fl_duration_time_str,'src_callsign':src_callsign,'max_altitude':max_altitude,'speed':0})
				print "Updated flight_log2", src_callsign
				cursor.execute('''UPDATE flight_log2 SET edate=?, etime=?, duration=?, max_altitude=?, speed=? WHERE src_callsign=?''',
							(fl_end_date,fl_end_time,fl_duration_time_str,max_altitude,0, src_callsign))
				print "Updated flight_log2", src_callsign
				nprev_vals[src_callsign]['speed'] = nvalues[src_callsign]['speed'] #ie set to '0'
				# Delete the record for this callsign from flight_log, ie flight complete
#				cursor.execute('''DELETE FROM flight_log2 WHERE src_callsign = ? ''', (src_callsign,))
				db.commit()	
				print "-----------------End of Packet: ", i, " ------------------------------"
				continue
			if nprev_vals[src_callsign]['speed'] == 0 and nvalues[src_callsign]['speed'] == 0:
				# Aircraft hasn't moved.  
