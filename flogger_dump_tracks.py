...
# 
# Is dump_tracks redundant? Replaced by dump_tracks2?
#-----------------------------------------------------------------
#
...
# timestamp, assuming the flarm units use GPS time for the timestamp.
#
#            cursor.execute('''SELECT latitude,longitude,altitude,course,speed,timeStamp 
#                                FROM track WHERE flight_no=? ORDER BY timeStamp''', (flight_no,))
...
#                 nxt_track.AddTrackPnt(longitude, latitude, altitude, str(timeStamp)) # timeStamp float to string
...
#                    cursor.execute('''SELECT latitude,longitude,altitude,course,speed,timeStamp
        ...
        #                timeStamp = track_point[7]
                        flight_nos = track_point["flight_no"]
                        track_nos = track_point["track_no"]
                        latitude = track_point["latitude"]
                        longitude = track_point["longitude"]
                        altitude = track_point["altitude"]
                        timeStamp = track_point["timeStamp"]
                        nxt_track.AddTrackPnt(longitude, latitude, altitude, strftime("%Y-%m-%d %H:%M:%S", localtime(timeStamp)))    # Convert timeStamp float to string
                    nxt_track.EndTrackSeg()
                    nxt_track.EndTrack()
